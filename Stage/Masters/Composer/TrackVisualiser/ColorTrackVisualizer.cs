...
namespace XanaduProject.Composer.TrackVisualiser
{
    public partial class ColorTrackVisualizer(Container editContainer) : TrackVisualiser<Color>
    {
        ...
        protected override void KeyFramePopup()
        {
            ...
            foreach (var child in editContainer.GetChildren()) child.QueueFree();
        ...
        public override void _Draw()
        {
            ...
            if (SelectedIndex != -1)
                DrawRect(new Rect2(new Vector2(points[SelectedIndex] * SPACING, 0) - rectSize * 0.6f, rectSize * 1.2f),
                    Colors.Yellow);

            DrawString(ThemeDB.FallbackFont, new Vector2(-25, 7), Index.ToString());
